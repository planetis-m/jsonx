...
proc lexNumber(input: string): TokKind =
  var p: JsonParser
  let s = streams.open(input)
  open(p, s, "test_numbers.json")
  defer: close(p)
  result = getTok(p)
  doAssert result in {tkInt, tkFloat}, "expected numeric token for " & input
...
block:
  let tk18 = lexNumber("123456789012345678")
  doAssert tk18 == tkInt

  let tk19 = lexNumber("1234567890123456789")
  doAssert tk19 == tkInt

  let tkBigFloat = lexNumber("0." & "1".repeat(40))
  doAssert tkBigFloat == tkFloat

  let tkBigExpFloat = lexNumber("9" & "0".repeat(40) & "e-5")
